...
public class GuildWebClient {

    @Autowired
    @Qualifier(WEB_CLIENT_GUILD_BEAN)
    ...
    public <T, R> R post(String url, T body, Class<R> responseType) {
        ...
        log.info(format("Запрос на сервер гильдии method = [%s], path = [%s], body = [%s]", POST, fullPath, toJson(body)));
    ...
    public <R> R get(String url, Class<R> responseType) {
        val fullPath = baseUrl + url;
        log.info(format("Запрос на сервер гильдии method = [%s], path = [%s]", GET, fullPath));
    ...
    public <T, R> R put(String url, T body, Class<R> responseType) {
        val fullPath = baseUrl + url;
        log.info(format("Запрос на сервер гильдии method = [%s], path = [%s], body = [%s]", PUT, fullPath, toJson(body)));
